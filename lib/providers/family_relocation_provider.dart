...
class FamilyRelocationProvider with ChangeNotifier{
  ...
  final FamilyRelocationService _familyRelocationService = FamilyRelocationService();
  ...
  Future<void> fetchFamilyRelocations(String token) async {
   ...
   Future<void> searchFamilyRelocations(String token, String query) async {
    ...
    try {
      _familyRelocations = await _familyRelocationService.searchFamilyRelocations(
        token,
        query,
      );
      _isLoading = false;
